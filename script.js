...
for (let i = 0; i < listaUtenti.length; i++) {
    let frase = `${listaUtenti[i].name} ${listaUtenti[i].lastName} ${listaUtenti[i].isAmbassador ? "è un Ambassador" : "non è un Ambassador"}`   
    console.log(frase);
}


// array solo utenti ambassador

const listaUtentiAmbassador = [];
for (let i = 0; i < listaUtenti.length; i++) {
    if (listaUtenti[i].isAmbassador) {
        listaUtentiAmbassador.push(listaUtenti[i]);
    }
}
console.table(listaUtentiAmbassador);